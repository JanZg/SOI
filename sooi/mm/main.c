...
/*Funkcja wywolujaca sie do samego mikrojadra za posrednictwem serwera MM */
PUBLIC int do_setprio(void)
{
	message msg;
	int result;
	msg = mm_in;
...
/* SYS_SETPRIO - zdefioniowana przez nas stala z com.h */
	result = _taskcall(SYSTASK, SYS_SETPRIO, &msg);
...
PUBLIC int do_getprio(void)
{
	message msg;
	int result;
	msg = mm_in;
	msg.m1_i1 = who;
	result = _taskcall(SYSTASK, SYS_GETPRIO, &msg);
	mm_in = msg; 
