...
FORWARD _PROTOTYPE( int do_getprio, (message *m_ptr) ); /* #SOI */
FORWARD _PROTOTYPE( int do_setprio, (message *m_ptr) ); /* #SOI */
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETPRIO:	r = do_getprio(&m);	break;
	    case SYS_SETPRIO:	r = do_setprio(&m);	break;
	    default:		r = E_BAD_FCN;
	}
...
/*
  ...
  if( (pp<BEG_USER_ADDR)||(pp>=END_PROC_ADDR) )
    return(ESRCH);
  if( ((m_ptr->m1_i2)!=PRIO_OFF)&&((m_ptr->m1_i2)!=PRIO_NORMALNY)&&((m_ptr->m1_i2)!=PRIO_TLO) )
    return(EINVAL);
  pp->p_subpriority = m_ptr->m1_i2;

  /*printf("___do_setprio: p_spri=%d; n_proces=%d;___\n", pp->p_subpriority, m_ptr->m1_i1);*/
  return(0);
}
