...
PRIVATE void sched()
{
  ...
  /*Wskazik na aktualnie wywlaszczany proces*/
  /*tmp = rdy_head[USER_Q]; */
  /*Przejscie po calej liscie i inkrementacja licznika zaglodzenia procesow tla*/
  /*Przypisanie do aktualnego wartosci nastepnego gotowego */
  /*tmp->licznik_starv = 0;*/
  /*while( (tmp=tmp->p_nextready) != NIL_PROC )
    tmp->licznik_starv++; */
  
  /* Zapamietuje aktualnie wywlaszczany proces zeby nie zapetlic listy */
  /* checkpoint = rdy_head[USER_Q]; */

/* Poki aktualnie wywlaszczony jest procesem tla i jesgo licznik zaglodzenia mniejszy od wartosci przypisanej */   
  /*do
  { */
    /* Aktualnie wywlaszczony proces z poczatku na koniec kolejki*/

/*printf("Sched %d / %d ",tmp->licznik_starv,MAXX); */



register struct proc *tmp, *checkpoint; 
  ...
  if (rdy_head[USER_Q] == NIL_PROC) return;
   ...
   if(tmp->licznik_starv<MAXX||tmp->p_subpriority==PRIO_NORMALNY)
{
...
while((tmp->p_subpriority==PRIO_NORMALNY)&&(tmp!=checkpoint))
{
...
tmp->licznik_starv=0;	
    ...
    /* Aktualny kandydat do wykonywania */
    /* tmp = rdy_head[USER_Q]; */
    /* Jesli nie ma kandydata to koniec*/
    /*if( (tmp == NIL_PROC) ) break;*/
    /* Jesli nie ma nastepnego gotowego to koniec */
    /* if( (tmp->p_nextready == NIL_PROC) ) break; */
    /* Jesli nastepny jest procesem ktory zrobiony byl jako pierwszy */
    /* if( tmp->p_nextready == checkpoint ) break; */

  /*}while( (tmp->p_subpriority==PRIO_TLO)&&(tmp->licznik_starv<MAXX) );

  tmp->licznik_starv = 0; */
  /*Podjecie procesu normalnego */
 
pick_proc();
  /*zerowanie licznika wykonanego programu */ 
